...
def read_npz_files(folder_path, npz_files, base_filename, keys_to_extract, cm_per_pixel, moving_average_window, max_gap, interpolate_nan=True, smooth=True):
    dataframes = []
    ...
    for idx, filename in enumerate(np.sort(npz_files)):
        ...
        # Handle the 2D array for segment ranges
        if "frame_segments" in npz_data:
            segment_data = npz_data["frame_segments"]
            segment_col = np.full(df.shape[0], np.nan)  # Initialize the segment column with NaN
        ...
        # Apply linear interpolation for NaNs in small gaps if enabled
        if interpolate_nan:
            columns_to_interpolate = df.columns.difference(['ID', 'frame', 'segment'])
            for column in columns_to_interpolate:
                df[column] = df[column].interpolate(method='linear', limit=max_gap, limit_direction='both')
    ...
    # Create unique tracklet IDs per trial 
    processed_df['tracklet'] = np.nan
    tracklet_counter = 1
    
    for trial in processed_df['Trial'].unique():
        trial_df = processed_df[processed_df['Trial'] == trial]
        unique_combinations = trial_df.groupby(['ID', 'segment']).size().index.tolist()
        
        tracklet_map = {comb: tracklet_counter + i for i, comb in enumerate(unique_combinations)}
        
        for (id_val, seg_val), tracklet_id in tracklet_map.items():
            processed_df.loc[(processed_df['Trial'] == trial) & (processed_df['ID'] == id_val) & (processed_df['segment'] == seg_val), 'tracklet'] = tracklet_id
        
        tracklet_counter += len(unique_combinations)
